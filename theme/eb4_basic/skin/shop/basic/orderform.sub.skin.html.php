...
.shop-order-form .payment-system-money-box {position:relative;overflow:hidden;clear:both;padding:10px 15px;border:1px solid #e5e5e5;border-top:0;background:#fff;text-align:right;color:#757575}
.shop-order-form .payment-system-money-box span {float:left}
.shop-order-form .payment-system-money-box strong {color:#000}
    ...
    <div class="order-member-payment">
        ...
        <div class="order-payment-area">
            ...
            <div class="payment-info-box">
                <span>보유머니 사용</span>
                <strong id="od_send_cost2"><?php echo $member['mb_system_money']; ?></strong>원
                <div class="text-gray m-t-10">
                    <input class="form-control" id="mb_system_money" name="mb_system_money" type="number" min="0" value="0" placeholder="사용할 머니금액을 입력해주세요" />
                </div>
            </div>
            ...
            <div class="order-payment-total">
                ...
                <div class="payment-system-money-box">
                    <span>보유머니 사용</span>
                    <strong><?php echo number_format(0); ?></strong>원
                </div>
...
$(function() {
    ...
    // 보유머니필드 변경시
    $(document).on('change', '#mb_system_money', function() {
        const total_money = <?php echo $member['mb_system_money']; ?>;
        const current_value = parseInt($(this).val());
        if (current_value > total_money) {
            $(this).val(total_money);
            $('.payment-system-money-box strong').text(total_money);
        } else {
            $('.payment-system-money-box strong').text(current_value);
        }
        
        calculate_total_price();
        const total_price = parseInt($("input[name=good_mny]").val());
        if (total_price < 0) {
            $(this).val(current_value + total_price);
            $('.payment-system-money-box strong').text(current_value + total_price);
        }
        calculate_total_price();
    });
});
...
function calculate_total_price()
{
    ...
    var send_cost = parseInt($("input[name=od_send_cost]").val());
    var use_system_money = parseInt($("#mb_system_money").val());
    ...
    tot_sell_price = sell_price - tot_cp_price + send_cost - use_system_money;
...
function calculate_order_price()
{
    ...
    var tot_price = sell_price + send_cost + send_cost2 - send_coupon - use_system_money;
