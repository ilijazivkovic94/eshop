...
$i_system_money  = isset($_POST['mb_system_money']) ? (int) $_POST['mb_system_money'] : 0;
...
if ($i_system_money)
{
    if ($member['mb_system_money'] < $i_system_money) {
        if(function_exists('add_order_post_log')) add_order_post_log('회원님의 시스템머니가 부족하여 시스템머니로 결제 할 수 없습니다.');
        alert('회원님의 시스템머니가 부족하여 시스템머니로 결제 할 수 없습니다.');
    }
}


$i_price = $i_price + $i_send_cost + $i_send_cost2 - $i_temp_point - $i_send_coupon - $i_system_money;
$order_price = $tot_od_price + $send_cost + $send_cost2 - $tot_sc_cp_price - $od_temp_point - $tot_od_price;
...
$sql = " insert {$g5['g5_shop_order_table']}
            set od_id             = '$od_id',
                ...
                od_system_money   = '$i_system_money',
...
// 회원이면서 시스템머니를 사용했다면 테이블에 사용을 추가
if ($is_member && $i_system_money)
    insert_system_money($member['mb_id'], (-1) * $i_system_money, "주문번호 $od_id 결제");
...
if ($friend && $friend['parent_mb_id']) {
    $friend_sql = "select * from g5_friends where mb_id='{$friend['parent_mb_id']}'";
    $friend_result = sql_query($friend_sql);
    $parent_friend = sql_fetch_array($friend_result);
    $member_sql = "select * from g5_member where mb_id='{$friend['parent_mb_id']}'";
    $member_result = sql_query($member_sql);
    $member_row = sql_fetch_array($member_result);
    if ($member_row) {
        $system_money = $member_row['mb_system_money'] + ceil($tot_ct_price * $friend['percentage'] / 100);
    ...
    if ($parent_friend && $parent_friend['parent_mb_id']) {
        $member_sql = "select * from g5_member where mb_id='{$parent_friend['parent_mb_id']}'";
        $member_result = sql_query($member_sql);
        $member_row = sql_fetch_array($member_result);
        if ($member_row) {
