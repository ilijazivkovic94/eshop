...
$login_code     = isset($_POST['login_code'])       ? trim($_POST['login_code'])     : "";
...
if ($login_code) {
    $sql = "select * from g5_friends where code='{$login_code}'";
    $result = sql_query($sql);
    for ($i=0; $row=sql_fetch_array($result); $i++) {
        if (!$row['mb_id']) {
            $update_sql = "update g5_friends SET mb_id='{$mb_id}' where code='{$login_code}'";
