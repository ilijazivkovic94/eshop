...
function insert_system_money($mb_id, $system_money, $content='', $rel_table='', $rel_id='', $rel_action='', $expire=0)
{
    ...
    // 회원머니
    $mb_money = $mb['mb_system_money'];

    // 이미 등록된 내역이라면 건너뜀
    if ($rel_table || $rel_id || $rel_action)
    {
        $sql = " select count(*) as cnt from g5_system_money
                  where mb_id = '$mb_id'
                    and po_rel_table = '$rel_table'
                    and po_rel_id = '$rel_id'
                    and po_rel_action = '$rel_action' ";
    ...
    $po_mb_money = $mb_money + $system_money;

    $sql = " insert into g5_system_money
                set mb_id = '$mb_id',
                    po_datetime = '".G5_TIME_YMDHIS."',
                    po_content = '".addslashes($content)."',
                    po_money = '$system_money',
                    po_use_money = '0',
                    po_mb_money = '$po_mb_money',
                    po_expired = '$po_expired',
                    po_expire_date = '$po_expire_date',
                    po_rel_table = '$rel_table',
                    po_rel_id = '$rel_id',
...
// 사용머니 입력
function insert_use_system_money($mb_id, $point, $po_id='')
{
    global $g5, $config;
